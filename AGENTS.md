...
- **Lazy re-exports**: packages that re-export torch-heavy names (`lerobot.policies`, `lerobot.utils`, `lerobot.lerobot_types`, `lerobot.rollout`, ...) import them under `if TYPE_CHECKING:` and list them in `else: __getattr__, __dir__ = lazy_exports(__name__, {...})` (see `utils/import_utils.py`), so importing the package stays light; `tests/utils/test_lazy_imports.py` checks that the two lists match. A module's own code imports such names where it uses them. `tests/test_torch_free_imports.py` fails if torch comes back on `lerobot-calibrate`, the find and setup commands, `lerobot-teleoperate`, `lerobot.configs`, `lerobot.policies`, `lerobot.processor` or `lerobot-rollout` (with the dataset extra).
